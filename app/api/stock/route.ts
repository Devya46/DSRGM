...
export async function GET(request: Request) {
  ...
  try {
    ...
    const symbol: string = bestMatch.symbol;
...
async function getStockData(symbol: string) {
  try {
    ...
    const [profileRes, quoteRes, historicalRes, financialsRes] =
      await Promise.all([
    ...
    return {
      ...
      netProfit: formatCurrency(
        financials.length > 0 ? financials[0].netIncome || 0 : 0,
      ),
