...
export function CharterAccountSection() {
  ...
  // Testimonials data (added more for smooth scrolling)
  const testimonials = [
    {
    ...
    // Duplicate for seamless looping (optional, but helps with continuous scroll)
    {
      text: "The mutual fund and equity advice I received was spot-on. Their team broke down complex terms into easy-to-understand guidance, and I've already seen positive returns thanks to their timely suggestions.",
      author: "Umesh Kumar Pitaliya",
      position: "Chief Manager SBI",
  ...
  useEffect(() => {
    const startAutoScroll = () => {
      ...
      scrollIntervalRef.current = setInterval(() => {
        if (testimonialScrollRef.current && !isPaused) {
          const { scrollLeft, scrollWidth, clientWidth } =
            testimonialScrollRef.current;
          const maxScroll = scrollWidth - clientWidth;
          const newScrollLeft = scrollLeft + 1; // Slow scrolling speed (1px per interval)
      ...
      }, 30); // Smooth, slow scrolling (adjust for speed)
    };
  ...
  // Auto-play functionality for services carousel
  useEffect(() => {
    if (isAutoPlaying) {
  ...
  const visibleServices = [0, 1, 2].map((offset) => {
    let index = currentCarouselIndex + offset;
  ...
  return (
    ...
    >
      ...
      {/* NEW: Full Width Scrolling Testimonials Section */}
      <div
          ...
          <div className="flex gap-6 px-4" style={{ width: "max-content" }}>
            {testimonials.map((testimonial, idx) => (
              <div
                ...
                className={`w-80 md:w-96 flex-shrink-0 rounded-2xl p-6 transition-all duration-300 hover:scale-105 ${
                  theme === "light"
